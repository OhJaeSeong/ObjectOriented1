...
Number *Integer::add(Number * num) {
	...
	other -> set_val(ex.first + value,ex.second);
	delete(this);
	return other;
    } else {
...
Number *Integer::sub(Number * num) {
     ...
     */
     if (num->types() == INTEGER) {
       Integer * other = dynamic_cast<Integer*>(num);
       other -> set_val(other->val() - value);
       delete(this); delete(num);
       return other;

    } else if (num->types() == FLOAT) {
        Float * other = dynamic_cast<Float*>(this);
        other -> set_val(other->val() - value);
        delete(this); delete(num);
        return other;

    } else if (num->types() == COMPLEX) {
        Complex * other = dynamic_cast<Complex*>(this);
        std::pair<double,double> ex = other->val();
        other -> set_val(ex.first - value,-ex.second);
        delete(this); delete(num);
        return other;
...
Number *Integer::mul(Number * num) {
     ...
     */
     if (num->types() == INTEGER) {
        ...
        Complex * other = dynamic_cast<Complex*>(num);
        std::pair<double,double> ex = other->val();
        other -> set_val(ex.first * value,ex.second*value);
        delete(this); delete(num);
        return other;
    } else {
