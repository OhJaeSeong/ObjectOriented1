...
Complex::Complex(float _val1, float _val2) {
    /**
     * td 네임 스페이스의 make_pair를 통해 pair 객체 생성 가능
...
Number *Complex::add(Number * num) {
	...
	c -> set_val(other->val() + this->real(), this->imag());
        delete(this); delete(other);
        return c;
	...
	Complex * c = new Complex();	
   	...
   	Complex * other = dynamic_cast<Complex*>(num);
        std::pair<double,double> ex = other->val();
	other -> set_val(ex.first + this->real(),this->imag());
	delete(this);
	return other;	
...
Number *Complex::mul(Number * num) {
 ...
 if (num->types() == INTEGER) {                              
        ...
        Complex * other = dynamic_cast<Complex*>(num);          
        std::pair<double,double> ex = other->val();
        other -> set_val(ex.first*(this->real()) - (this->imag())*(ex.second),ex.second*(this->real()) + (this->imag())*ex.first);
        delete(this);
        return other;                                           
